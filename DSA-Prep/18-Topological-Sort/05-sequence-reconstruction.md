...
public boolean sequenceReconstruction(int[] org,List<List<Integer>> seqs){
    Map<Integer,Set<Integer>>g=new HashMap<>();Map<Integer,Integer>in=new HashMap<>();for(int x:org){g.put(x,new HashSet<>());in.put(x,0);}
    ...
    while(!q.isEmpty()){if(q.size()!=1)return false;int u=q.poll();if(u!=org[k++])return false;for(int v:g.get(u))if(--in.put(v,in.get(v)-1)==0)q.offer(v);}return k==org.length;
}
```
